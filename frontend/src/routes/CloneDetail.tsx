...
export function CloneDetail() {
    ...
    const [deleteSnapFor, setDeleteSnapFor] = useState<SnapshotItem | null>(null)
    const [retentionFor, setRetentionFor] = useState<SnapshotItem | null>(null)
    ...
    const confirmDeleteSnapshot = useCallback(async () => {
        if (!deleteSnapFor) return
        setSnapBusy(true)
        const tid = toast.loading('Deleting snapshot…')
        try {
            const r = await fetch(`${base}/snapshots/${encodeURIComponent(deleteSnapFor.name)}`, { method: 'DELETE' })
            if (!r.ok) throw new Error(`${r.status} ${await r.text()}`)
            toast.update(tid, 'success', 'Snapshot deleted')
            setDeleteSnapFor(null)
            await Promise.all([fetchCloneSnapshots(), fetchAllSnapshots()])
        } catch (e: any) {
            toast.update(tid, 'error', `Delete failed: ${String(e?.message || e)}`)
        } finally {
            setSnapBusy(false)
        }
    }, [base, deleteSnapFor, fetchCloneSnapshots, fetchAllSnapshots, toast])

    const openRetention = useCallback((s: SnapshotItem) => {
        const rd = s.metadata?.retention_days
        setRetentionDays(typeof rd === 'number' ? rd : 14)
        setRetentionFor(s)
    }, [])
    ...
    const confirmRetention = useCallback(async () => {
        ...
        setSnapBusy(true)
        const tid = toast.loading('Updating retention…')
        try {
            const r = await fetch(`${base}/snapshots/${encodeURIComponent(retentionFor.name)}/retention`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!r.ok) throw new Error(`${r.status} ${await r.text()}`)
            const res = await r.json()
            toast.update(tid, 'success', res.expires_at ? `Retention updated — expires ${new Date(res.expires_at).toLocaleDateString()}` : 'Retention updated — keeps forever')
            setRetentionFor(null)
            await Promise.all([fetchCloneSnapshots(), fetchAllSnapshots()])
        } catch (e: any) {
            toast.update(tid, 'error', `Retention update failed: ${String(e?.message || e)}`)
        } finally {
            setSnapBusy(false)
        }
    }, [base, retentionFor, retentionDays, fetchCloneSnapshots, fetchAllSnapshots, toast])
    ...
    const selectedSnap = (name: string | null): SnapshotItem | null =>
        name ? allSnapshots.find((s) => s.name === name) ?? null : null

    const expiryText = (s: SnapshotItem | null): string => {
        if (!s) return ''
        const rd = s.metadata?.retention_days
        const exp = s.metadata?.expires_at
        if (rd === 0) return 'keeps forever'
        if (exp) return `expires ${new Date(exp).toLocaleDateString()}`
        return ''
    }
    ...
    return (
        <div className="mx-auto max-w-5xl animate-page-in px-6 pb-20 pt-6">
                ...
                <div className="flex items-start justify-between gap-3">
                    ...
                    {nodeActionFor && mySnapshots.some((s) => s.name === nodeActionFor) && (
                        <div className="flex flex-none flex-wrap items-center justify-end gap-2">
                            {expiryText(selectedSnap(nodeActionFor)) && (
                                <span className="text-[11px] text-muted-foreground">{expiryText(selectedSnap(nodeActionFor))}</span>
                            )}
                            <Button variant="primary" onClick={() => { const n = nodeActionFor; setNodeActionFor(null); if (n) setResetFor(n) }} disabled={moveBusy || snapBusy}>
                                Reset to this snapshot
                            </Button>
                            <Button onClick={() => { const s = selectedSnap(nodeActionFor); setNodeActionFor(null); if (s) openRetention(s) }} disabled={moveBusy || snapBusy}>
                                Set retention
                            </Button>
                            <Button variant="destructive" onClick={() => { const s = selectedSnap(nodeActionFor); setNodeActionFor(null); if (s) setDeleteSnapFor(s) }} disabled={moveBusy || snapBusy}>
                                Delete
                            </Button>
                            <Button onClick={() => setNodeActionFor(null)} disabled={moveBusy || snapBusy}>Done</Button>
                        </div>
                    )}
                </div>
                <LineageGraph
            ...
            <Dialog open={!!deleteSnapFor} onOpenChange={(open) => { if (!open && !snapBusy) setDeleteSnapFor(null) }}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button onClick={() => setDeleteSnapFor(null)} disabled={snapBusy}>Cancel</Button>
                        <Button variant="destructive" onClick={confirmDeleteSnapshot} disabled={snapBusy}>
                            {snapBusy ? 'Deleting...' : 'Delete'}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            <Dialog open={!!retentionFor} onOpenChange={(open) => { if (!open && !snapBusy) setRetentionFor(null) }}>
                <DialogContent>
                    <DialogTitle>Set retention</DialogTitle>
                    <DialogDescription>
                        How long this snapshot is kept before it becomes eligible for cleanup. Counted from its creation time; “keep forever” never expires.
                    </DialogDescription>
                    <div className="mt-2 break-all rounded-md border border-border bg-secondary px-3 py-2 font-mono text-[12px] text-muted-foreground">
                        {retentionFor?.description?.trim() || retentionFor?.name}
                    </div>
                    <div className="mt-3">
                        <RetentionSelect value={retentionDays} onChange={setRetentionDays} />
                    </div>
                    <DialogFooter>
                        <Button onClick={() => setRetentionFor(null)} disabled={snapBusy}>Cancel</Button>
                        <Button variant="primary" onClick={confirmRetention} disabled={snapBusy}>
