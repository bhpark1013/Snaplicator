...
def get_clone_usage_summary(root_data_dir: str, main_data_dir: str, identifier: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "usage_bytes": entry.get("exclusive_bytes"),
        "total_bytes": entry.get("total_bytes"),
        "measured_at": entry.get("measured_at"),
        "stale": entry.get("stale"),
        "refreshing": entry.get("refreshing"),
...
def set_snapshot_retention(
    root_data_dir: str,
    snapshot_name: str,
    retention_days: int,
) -> Dict[str, Any]:
    """Change a snapshot's retention. expires_at is recomputed from the snapshot's
    original created_at (so retention_days means "days from creation", identical to
    create time); retention_days <= 0 keeps it forever. Snapshots are readonly
    subvolumes, so we briefly toggle ro=false to rewrite the metadata, then back.
    No PG data is touched."""
    root = Path(root_data_dir).resolve()
    target = (root / snapshot_name).resolve()
    if not str(target).startswith(str(root)):
        raise PermissionError(f"Refusing to edit outside ROOT_DATA_DIR: {target}")
    ...
    meta.update(_retention_fields(created, retention_days))
