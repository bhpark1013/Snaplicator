...
**macOS** — no `sudo`, and nothing to set up first:
...
orb -m snaplicator docker ps    # its containers — plain `docker ps` will not show them
```
...
thing work without having a database ready. Both can be given up front, which
skips the prompt:
...
it, starts the management plane, and opens the UI. It stops there: **what to
replicate is chosen in the UI**, and the first copy starts when you say so.
`START_REPLICATION=1` replicates everything without asking.
